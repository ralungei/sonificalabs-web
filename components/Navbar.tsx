...
export function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 md:px-8 h-14 transition-[background-color,backdrop-filter] duration-300 ${scrolled ? "bg-surface-0/80 backdrop-blur-xl border-b border-white/[0.04]" : "bg-transparent border-b border-transparent"}`}
      >
        {/* Left: Logo + nav links */}
        <div className="flex items-baseline gap-8">
          <Link href="/" className="flex items-baseline gap-2 select-none">
            ...
            <span className="text-[10px] text-white/80 leading-none -ml-1">by Ras</span>
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          {status === "loading" ? (
            <div className="hidden md:block h-8 w-8 rounded-full bg-white/[0.06] animate-pulse" />
          ) : session?.user ? (
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center w-8 h-8 rounded-lg text-white/70 hover:text-white hover:bg-white/[0.06] transition-colors"
            aria-label="Menu"
          >
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 z-[56] w-64 bg-surface-1/95 backdrop-blur-xl border-l border-white/[0.06] flex flex-col md:hidden"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-5 h-14 border-b border-white/[0.06]">
                <span className="text-sm font-body font-semibold text-white/80">Menu</span>
                <button
                  onClick={() => setMobileOpen(false)}
              ...
              <div className="flex flex-col gap-1 px-3 py-4">
                <a
                  ...
                  className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-body text-text-secondary hover:text-text-primary hover:bg-white/[0.06] transition-colors"
                >
                  <Icon icon="solar:lightbulb-bolt-linear" className="h-5 w-5 text-white/40" />
                  {t("howItWorks")}
                </a>
                <Link
                  href="/pricing"
                  ...
                  className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-body text-text-secondary hover:text-text-primary hover:bg-white/[0.06] transition-colors"
                >
                  <Icon icon="solar:tag-price-linear" className="h-5 w-5 text-white/40" />
                  {t("pricing")}
                </Link>
                <Link
                  href="/about"
                  ...
                  className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-body text-text-secondary hover:text-text-primary hover:bg-white/[0.06] transition-colors"
                >
                  <Icon icon="solar:users-group-rounded-linear" className="h-5 w-5 text-white/40" />
                  {t("about")}
                </Link>
              ...
              <div className="mt-auto border-t border-white/[0.06] px-3 py-4">
                <div className="pb-3 mb-3 border-b border-white/[0.06] px-3">
                  <div className="relative">
                    <Icon icon="solar:global-linear" className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-white/40 pointer-events-none" />
                    <select
                      value={locale}
                      onChange={(e) => {
                      ...
                      className="w-full appearance-none bg-surface-0/60 border border-white/[0.08] rounded-xl pl-10 pr-10 py-2.5 text-sm font-body text-text-primary focus:outline-none focus:border-accent/30 transition-colors cursor-pointer"
                    >
                      {locales.map((l) => (
                    ...
                    <svg className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                    </svg>
                  </div>
                </div>
                {session?.user ? (
                  <>
                    <Link
                      href="/account"
                      onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-3 px-3 py-2.5 mb-1 rounded-xl hover:bg-white/[0.06] transition-colors"
                    >
                      {session.user.image ? (
                    ...
                    <button
                      ...
                      className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-body text-text-secondary hover:text-text-primary hover:bg-white/[0.06] transition-colors"
                    >
                      <Icon icon="solar:logout-2-linear" className="h-5 w-5 text-white/40" />
                      {t("signOut")}
                    </button>
                  </>
                ) : (
